    ...
    inner class StarRenderer : GLSurfaceView.Renderer {
        ...
        private fun generateStars(count: Int) {
            for (i in 0 until count) {
                ...
                val z = cos(phi)
                stars.add(Star(floatArrayOf(x, y, z)))
            }
        }
    }
...
data class Star(val position: FloatArray) {
    fun draw(program: Int) {
        val positionHandle = GLES20.glGetAttribLocation(program, "aPosition")
        GLES20.glEnableVertexAttribArray(positionHandle)
        GLES20.glVertexAttrib3fv(positionHandle, position, 0)
